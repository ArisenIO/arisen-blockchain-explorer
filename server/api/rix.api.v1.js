...
const customFunctions = require('./rix.api.v1.custom');
...
module.exports 	= (router, config, request, log, mongoMain, MARIA) => {
	...
	router.post('/api/v1/search', (req, res) => {
		...
		async.parallel({
			block: (cb) =>{
        		...
        		global.rix.getBlock({ block_num_or_id: text })
	   			 	.then(result => {
	   			 		cb(null, result);
	   			 	})
	   			 	.catch(err => {
			...
			transaction: (cb) =>{
				console.log("TRX", text);
				global.rix.getTransaction({ id: text })
	   			 	.then(result => {
	   			 		cb(null, result);
	   			 	})
	   			 	.catch(err => {
			...
			account: (cb) =>{
				...
				global.rix.getAccount({ account_name: text })
	   			 	.then(result => {
	   			 		cb(null, result);
	   			 	})
	   			 	.catch(err => {
			...
			key: (cb) => {
				...
				global.rix.getKeyAccounts({ public_key: text })
	   	 			.then(result => {
	   	 				cb(null, result);
	   	 			})
	   	 			.catch(err => {
			...
			contract: (cb) =>{
				...
				global.rix.getCode({ json: true, account_name: text })
	   	 			.then(result => {
	   	 				cb(null, result)
	   	 			})
	   	 			.catch(err => {
	...
	*/
	router.get('/api/v1/get_block/:block_num_or_id', (req, res) => {
	   	 global.rix.getBlock({ block_num_or_id: req.params.block_num_or_id })
	   	 	.then(result => {
	   	 		res.json(result);
	   	 	})
	   	 	.catch(err => {
	...
	*/
	router.get('/api/v1/get_last_blocks/:offset', (req, res) => {
		...
		customFunctions.getLastBlocks(rix, elements, (err, result) => {
				if (err){
					log.error(err);
					return res.status(501).end();
				}
	...
	*/
	...
	router.post('/api/producer', (req, res) => {
		if (req.body.url && req.body.url.indexOf('rixweb.net') >= 0 ){
